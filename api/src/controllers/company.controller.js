...
exports.create = async (req, res) => {
  ...
  // Create a Company
    const company = new Company({
        ...
        settings: {
            features: {
                leave: req.body.settings?.features?.leave,
                payroll: req.body.settings?.features?.payroll,
                ...
                events: req.body.settings?.features?.events,
                ...
                jobManagement: req.body.settings?.features?.jobManagement,
                holidayManagement: req.body.settings?.features?.holidayManagement,
                projectManagement: req.body.settings?.features?.projectManagement
            },
  ...
  company
    .save(company)
    .then(data => {
      this.sendVerificationEmail(data.email, data.settings.emailVerificationCode, data.name);
...
exports.verifyEmail = async (req, res) => {
  ...
  const company = await Company.findOne({ 'settings.emailVerificationCode': token });
  ...
  if (company?.settings.emailVerificationExpiry < new Date()) {
    ...
    await Company.updateOne({ _id: company._id }, { $set: { 'settings.emailVerificationCode': newToken, 'settings.emailVerificationExpiry': expiryDate } });
  ...
  if (company) {
    ...
    company.save()
      ...
      .catch(err => {
        res.status(500).send({ message: err.message || "Some error occurred, please try again" });
      });
  } else {
...
exports.resendVerificationEmail = async (req, res) => {
  ...
  if (company) {
    ...
    // send new email verification link
    this.sendVerificationEmail(company.email, newToken, company.name);
    res.status(200).send({ status: 200, message: "Email verification link has been sent" });
  } else {
    res.status(200).send({ status: 404, message: "Error please try again !" });
  }
};
...
exports.findOne = (req, res) => {
  ...
  Company.findOne({ _id: id })
    .then(data => {
      if (!data)
        res.status(200).send({ status: 404, message: "Not found Company with id " + id });
...
exports.updateCompanyFLWAccount =  (accountDetails) => {
  ...
  Company.findByIdAndUpdate(account.companyId, { bankAccount: account }, { useFindAndModify: true })
    .then(data => {
      if (!data) {
