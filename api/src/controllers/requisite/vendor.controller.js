...
// employee vendor
exports.employeeVendor = (req, res) => {
  if(!req.headers.authorization) {
    return res.status(401).send({ message: "Unauthorized request" });
  }
    Vendor.find({ employee_id: req.params.employee_id })
    .then(data => {
        const sortedData = data.sort((a, b) => {
            return new Date(b.createdAt) - new Date(a.createdAt);
        });
    ...
    .catch(err => {
        res.status(500).send({
            message:
                err.message || "Some error occurred while retrieving employee vendors."
        });
    });
};
...
exports.count = (req, res) => {
  ...
  .then(data => {
    res.status(200).json({ totalVendors: data });
  })
  .catch(err => {
    res.status(500).json({
      message:
